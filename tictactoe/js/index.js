(function () {
    ...
    function functionLoad() {
        ...
        addEvent(document.getElementById("game"), "click", nextMove);
        document.getElementById("resetBtn").addEventListener("click", tictactoeNewGame);
    ...
    function tictactoeNewGame() {
        ...
        for (i = 1; i <= tictactoeSize; i += 1) {
            var tr = document.createElement('tr');
            for (j = 1; j <= tictactoeSize; j += 1) {
                k += 1;
    ...
    function findWinner(board, prevTurn, turn) {
        if (score[turn] >= tictactoeSize) {
            if (board[0][0] === prevTurn && board[0][1] === prevTurn && board[0][2] === prevTurn){
                alert(turn + ' congratulations to You!');
                tictactoeNewGame();
            }
        ...
        } else if ((score['X'] + score['O']) === (tictactoeSize * tictactoeSize)) {
            alert('Draw!');
            tictactoeNewGame();
        } else {
            return false;
